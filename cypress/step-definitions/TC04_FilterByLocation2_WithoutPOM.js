    ...
    cy.request(EVENT_SERVICE_SEARCH_FILTERS)
      .then((response) => {
          ...
          var locations = specialties.map(function(item) {
            ...
            home.getSpecialtyFilterListBox().contains($loc).should('exist')
        })
    })    
})
...
Then('User should see only one matching row containing {string} in list box', (location) => {
    //cy.get('.v-select-list').find('.v-list-item__content').then((elem) => {
    home.getSpecialtyFilterListBox().then((elem) => {
      //cy.get('.v-select-list').find('.v-list-item__content').invoke('text').should('eq', location)
      home.getSpecialtyFilterListBox().invoke('text').should('eq', location)
...
Then('User should see the message as {string} in list box', (message) => {
    //cy.get('.v-select-list').find('.v-list-item__content').invoke('text').should('eq', message)
    home.getSpecialtyFilterListBox().invoke('text').should('eq', message)
})
...
And('User selects the second -{int}- option from list-box', (index) => {
    //cy.get('[data-test=live-event-search__location-option]').then(() => {
    home.getLocationFilter().then(() => {
      //cy.get('[role=listbox] > div:visible').eq(index-1).then((el) => {
      home.getSpecialtyFilterListBox().eq(index-1).then((el) => {
        myLocation = el.text()
      })
      //cy.get('[role=listbox] > div:visible').eq(index-1).click()
      home.getSpecialtyFilterListBox().eq(index-1).click()
    })    
})

And('User sees the URL parameter as - specialty={string}&location=SECOND-OPTION', (specialty) => {
  var location = myLocation.replace(" ", "%20");
  cy.url()
...
And('User selects option as {string}', (specialty) => {
    //cy.get('[data-test=live-event-search__specialty-option]').click().then(() => {
    home.getSpecialtyFilterListBox().click().then(() => {
      //cy.get('[role=listbox]')
      home.getSpecialtyFilterListBox()
      .contains(specialty)
      .click()
    })    
})
